...
import { ENGLISH_LEVELS, generateText, type AgeTier } from "../data/english";
import {
...
const TIER_LABELS: Record<AgeTier, { label: string; ages: string }> = {
  little:   { label: "🟢 Little Buddy",   ages: "5–7" },
  explorer: { label: "🟡 Explorer Buddy", ages: "8–10" },
  pro:      { label: "🔴 Pro Buddy",      ages: "11–13" },
};

const WORD_COUNTS: Record<AgeTier, number> = { little: 20, explorer: 25, pro: 30 };
...
export default function EnglishTypingPage() {
  ...
  const levelId = ENGLISH_LEVELS[levelIdx].id;

  const newExercise = useCallback((lId: string, t: AgeTier) => {
    const pool = ENGLISH_LEVELS.find((l) => l.id === lId) ?? ENGLISH_LEVELS[0];
    setExerciseText(generateText(pool, t, WORD_COUNTS[t]));
    setResult(null);
    setExerciseKey((k) => k + 1);
  }, []);

  // Init exercise on mount
  useEffect(() => {
    if (mounted) newExercise(ENGLISH_LEVELS[0].id, "explorer");
  ...
  const handleComplete = useCallback((r: TypingResult) => {
    ...
    setPrevBest(getPersonalBest(user, levelId, "english"));
  ...
  const handleLevelChange = (idx: number) => {
    setLevelIdx(idx);
    newExercise(ENGLISH_LEVELS[idx].id, tier);
  };
  const handleRetry = () => newExercise(levelId, tier);
  const handleNextLevel = () => {
    const next = Math.min(levelIdx + 1, ENGLISH_LEVELS.length - 1);
    setLevelIdx(next);
    newExercise(ENGLISH_LEVELS[next].id, tier);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-sky-50 to-indigo-50">
      <div className="sticky top-0 z-20 bg-white/95 backdrop-blur border-b border-gray-100 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <Link href="/typing/game" className="text-xs font-black bg-yellow-100 text-yellow-700 px-3 py-1.5 rounded-full hover:bg-yellow-200 transition-colors">
              🎮 Word Rain
            </Link>
        ...
        <div className="bg-blue-500 px-4 py-2">
          <div className="max-w-4xl mx-auto flex items-center justify-between gap-3">
            <span className="text-white font-black">⌨️ English Typing</span>
            <div className="flex gap-1">
              {(Object.entries(TIER_LABELS) as [AgeTier, { label: string; ages: string }][]).map(([t, cfg]) => (
                <button key={t} onClick={() => handleTierChange(t)}
                  className={`px-3 py-1 rounded-full text-xs font-black transition-all ${tier === t ? "bg-white text-blue-600 shadow-sm" : "text-white/80 hover:text-white"}`}>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        <div className="flex gap-2 mb-6 overflow-x-auto pb-1 scrollbar-hide">
          {ENGLISH_LEVELS.map((lv, idx) => (
            <button key={lv.id} onClick={() => handleLevelChange(idx)}
              className={`flex-shrink-0 px-4 py-2 rounded-xl text-xs font-black transition-all border ${idx === levelIdx ? "bg-blue-500 text-white border-blue-600 shadow-md scale-105" : "bg-white text-gray-600 border-gray-200 hover:border-blue-300"}`}>
              {lv.label}
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="font-black text-gray-800 text-lg">
            {ENGLISH_LEVELS[levelIdx].label}
            <span className="ml-2 text-sm font-bold text-gray-400">{ENGLISH_LEVELS[levelIdx].sublabel}</span>
          </h2>
          <button onClick={() => setShowKeyboard(!showKeyboard)}
            ...
            ⌨️ {uiLang === "en" ? "Keyboard" : "키보드"} {showKeyboard ? "ON" : "OFF"}
          </button>
        </div>

        {result ? (
          <ResultCard
            result={{ levelId, language: "english", wpm: result.wpm, accuracy: result.accuracy, durationMs: result.durationMs, timestamp: Date.now() }}
            prevBest={prevBest}
          ...
          <TypingArea key={exerciseKey} text={exerciseText} language="english"
        ...
        {user && (
          <p className="text-center text-xs font-bold text-gray-400 mt-6">
            👤 {user} · <Link href="/typing/stats" className="text-blue-500 hover:underline">
